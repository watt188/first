...
class LiveRecoveryOrchestratorV84:
    """Turn an exact-head GitHub failure snapshot into one bounded recovery action.

    The orchestrator fails closed when multiple simultaneous failures imply
    different recovery actions. It delegates the actual mutation to V8.3 and
    therefore cannot merge, bypass the Ruleset, or write to main.
    """
    ...
    @staticmethod
    def _decision_for(failure: WorkflowFailureV84, attempts: int, max_attempts: int):
        signal = FailureSignal(failure.workflow, failure.conclusion, failure.log_excerpt)
        return AutonomousRecoveryV82.next_action(signal, attempts, max_attempts)
    ...
    def recover(self, snapshot: RecoverySnapshotV84) -> dict:
        ...
        for failure in snapshot.failures:
            ...
            decision = self._decision_for(failure, attempts, snapshot.max_attempts)
            assessed.append((failure, attempts, decision))

        actions = {decision.action for _, _, decision in assessed}
        categories = {decision.category for _, _, decision in assessed}
