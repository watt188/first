...
class ProtectedDeliverySpecV79:
    goal: str
    base_sha: str
...
class ProtectedAutonomousDeliveryV79:
    """Compose autonomous generation, evidence verification, policy and PR opening.

    This controller never merges or bypasses GitHub. It opens a PR only after the
    autonomous output is PR-ready, exact-base evidence verifies, and the live
    server ruleset snapshot satisfies the V7.6 fail-closed contract.
    """
    ...
    def deliver(self, spec: ProtectedDeliverySpecV79, ruleset: dict) -> dict:
        spec.validate()
        ...
        opened = self.bridge.deliver(DeliverySpec(goal=spec.goal, base_sha=spec.base_sha, base_branch="main"), files)
        if opened.get("status") != "PR_OPENED":
            raise RuntimeError("github_delivery_failed")
        return {"status":"PR_OPENED_PROTECTED","version":VERSION,"base_sha":spec.base_sha,"policy":policy,"evidence_verified":True,"delivery":opened}
