...
class Runner:
    ...
    def run(self, goal: str):
        ...
        report.write_text(json.dumps({"goal": goal}), encoding="utf-8")
        manifest = artifacts / "manifest.json"
        seal(
            self.root,
            ["artifacts/report.json", "generated/feature_v65.py", "generated/test_feature_v65.py"],
            manifest,
            self.source_sha,
        )
        return {
            ...
            "manifest": "artifacts/manifest.json",
            "outputs": ["generated/feature_v65.py", "generated/test_feature_v65.py"],
        }
...
def test_protected_delivery_opens_pr_only_after_policy_and_evidence(tmp_path):
    gateway = Gateway()
    controller = ProtectedAutonomousDeliveryV79(tmp_path, Runner(tmp_path), gateway)
    result = controller.deliver(ProtectedDeliverySpecV79("ship bounded feature", BASE_SHA), _ruleset())
...
def test_rejects_policy_drift_before_runner_executes(tmp_path):
    ...
    with pytest.raises(ValueError, match="strict_status_checks_disabled"):
        controller.deliver(ProtectedDeliverySpecV79("ship bounded feature", BASE_SHA), ruleset)
