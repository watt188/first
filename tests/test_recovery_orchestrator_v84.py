...
from protected_delivery.recovery_orchestrator_v84 import (
    LiveRecoveryOrchestratorV84,
    RecoverySnapshotV84,
    WorkflowFailureV84,
)
...
def test_single_provider_failure_reruns_exact_job():
    gateway = Gateway()
    result = LiveRecoveryOrchestratorV84(gateway).recover(
        RecoverySnapshotV84(
            26,
            HEAD,
            HEAD,
            (WorkflowFailureV84("v70-production-convergence", "failure", 99, "provider_circuit_open"),),
        )
    )
...
def test_single_code_failure_requests_rework():
    gateway = Gateway()
    result = LiveRecoveryOrchestratorV84(gateway).recover(
        RecoverySnapshotV84(26, HEAD, HEAD, (WorkflowFailureV84("ci", "failure", 8, "assertion failed"),))
    )
...
def test_stale_head_fails_before_action():
    ...
    with pytest.raises(RuntimeError, match="stale_pr_head"):
        LiveRecoveryOrchestratorV84(gateway).recover(
            RecoverySnapshotV84(26, HEAD, "b" * 40, (WorkflowFailureV84("ci", "failure", 8),))
        )
    assert gateway.calls == []


def test_multiple_failures_with_mixed_actions_escalate():
    gateway = Gateway()
    result = LiveRecoveryOrchestratorV84(gateway).recover(
        RecoverySnapshotV84(
            26,
            ...
            HEAD,
            (
                WorkflowFailureV84("ci", "failure", 8, "assertion failed"),
                WorkflowFailureV84("v70-production-convergence", "failure", 9, "provider_circuit_open"),
            ),
        )
    )
