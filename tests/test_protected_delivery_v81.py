...
from protected_delivery.v81 import ProtectedAutonomousDeliveryV81, ProtectedDeliverySpecV81
...
def test_protected_delivery_opens_pr_only_after_policy_and_evidence(tmp_path):
    gateway = Gateway()
    result = ProtectedAutonomousDeliveryV81(tmp_path, Runner(tmp_path), gateway).deliver(ProtectedDeliverySpecV81("ship bounded feature", BASE_SHA), _ruleset())
...
def test_rejects_stale_autonomous_evidence(tmp_path):
    controller = ProtectedAutonomousDeliveryV81(tmp_path, Runner(tmp_path, "c" * 40), Gateway())
    with pytest.raises(RuntimeError, match="delivery_source_sha_mismatch"):
        controller.deliver(ProtectedDeliverySpecV81("ship bounded feature", BASE_SHA), _ruleset())
...
def test_rejects_policy_drift_before_runner_executes(tmp_path):
    ...
    with pytest.raises(ValueError, match="strict_status_checks_disabled"):
        ProtectedAutonomousDeliveryV81(tmp_path, Runner(tmp_path), Gateway()).deliver(ProtectedDeliverySpecV81("ship bounded feature", BASE_SHA), ruleset)


def test_rejects_unexpected_repository():
    with pytest.raises(ValueError, match="unexpected_repository"):
        ProtectedDeliverySpecV81("x", BASE_SHA, "other/repo").validate()
